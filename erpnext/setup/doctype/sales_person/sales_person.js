...
frappe.ui.form.on("Sales Person", {
		...
		if (frm.doc.__onload && frm.doc.__onload.dashboard_info) {
			...
			frm.dashboard.add_indicator(
				__("Total Contribution Amount Against Orders: {0}", [
					format_currency(
						info.allocated_amount_against_order,
						info.currency,
					),
				]),
				"blue",
			...
			frm.dashboard.add_indicator(
				__("Total Contribution Amount Against Invoices: {0}", [
					format_currency(
						info.allocated_amount_against_invoice,
						info.currency,
					),
				]),
				"blue",
	...
	setup: function (frm) {
		frm.fields_dict["targets"].grid.get_field("distribution_id").get_query =
...
cur_frm.fields_dict["parent_sales_person"].get_query = function (
	doc,
	cdt,
	cdn,
) {
