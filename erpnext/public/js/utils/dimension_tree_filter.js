...
erpnext.accounts.dimensions = {
	...
	fetch_custom_dimensions(frm, doctype) {
		...
		frappe.call({
			...
			callback: function (r) {
				...
				me.accounting_dimensions = me.accounting_dimensions.filter(
					(x) => {
		...
		if (frappe.meta.has_field(doctype, dimension)) {
			frappe.model.with_doctype(doctype, () => {
				...
				frm.set_query(dimension, parentfield, function (doc, cdt, cdn) {
					...
					return erpnext.queries.get_filtered_dimensions(
						row,
						fields,
						dimension,
						doc.company,
				...
				if (frm.is_new()) {
					if (
						...
						if (default_dimension) {
							...
							$.each(
								frm.doc.items || frm.doc.accounts || [],
								function (i, row) {
	...
	copy_dimension_from_first_row(frm, cdt, cdn, fieldname) {
		if (
			frappe.meta.has_field(frm.doctype, fieldname) &&
			this.accounting_dimensions
		) {
			this.accounting_dimensions.forEach((dimension) => {
				let row = frappe.get_doc(cdt, cdn);
				frm.script_manager.copy_from_first_row(fieldname, row, [
					dimension["fieldname"],
				]);
			});
		}
