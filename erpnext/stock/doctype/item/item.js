...
frappe.ui.form.on("Item", {
	...
	refresh: function (frm) {
		...
		frm.add_custom_button(__("Duplicate"), function () {
			...
			if (
				new_item.item_code === new_item.description ||
				new_item.item_code === new_item.description
			) {
	...
	set_asset_naming_series: function (frm, asset_naming_series) {
		if (
			(frm.doc.__onload && frm.doc.__onload.asset_naming_series) ||
			asset_naming_series
		) {
			let naming_series =
				(frm.doc.__onload && frm.doc.__onload.asset_naming_series) ||
...
$.extend(erpnext.item, {
	setup_queries: function (frm) {
		...
		frm.fields_dict["taxes"].grid.get_field("tax_type").get_query =
		...
		frm.fields_dict.reorder_levels.grid.get_field("warehouse").get_query =
			function (doc, cdt, cdn) {
						...
						onchange: function () {
							...
							if (lengths.includes(0)) {
								...
								if (no_of_combinations === 1) {
									msg = __("Make {0} Variant", [
										no_of_combinations,
									]);
								} else {
									msg = __("Make {0} Variants", [
										no_of_combinations,
									]);
								}
								me.multiple_variant_dialog
			...
			me.multiple_variant_dialog.set_primary_action(
				...
				() => {
					...
					let use_template_image =
						me.multiple_variant_dialog.get_value(
							"use_template_image",
						);
...
function open_form(frm, doctype, child_doctype, parentfield) {
	frappe.model.with_doctype(doctype, () => {
		...
		let new_child_doc = frappe.model.add_child(
			new_doc,
			child_doctype,
			parentfield,
		);
		...
		if (in_list(SALES_DOCTYPES, doctype) && frm.doc.sales_uom) {
			...
			in_list(PURCHASE_DOCTYPES, doctype) &&
